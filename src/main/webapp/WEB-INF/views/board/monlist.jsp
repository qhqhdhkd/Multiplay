...
<jsp:include page="/WEB-INF/views/common/header.jsp">
	<jsp:param value="멀티플레이::문의사항" name="title"/>
</jsp:include>
...
<body>
	<section>
		<div class="title">
			<span class="title_name" id="title_name">문의사항</span>
		</div>
	</section>
		...
		<div class="container">
			<div class="content-container">
				<article id="tab1" class="content-container__content target">
					<form action="${path}/board/monlist" method="get" class="search">
								...
								<tr>
									...
									<td class="board-content board-title">
										<a id="board-title" href="${path}/board/view?no=${board.bno}"> 
											<c:out value="[문의]  ${board.boardTitle}" />
										</a>
									</td>
					...
					<div class="table-buttons">
						<button onclick="movePage('${path}/board/monlist?page=1');" class="table-button">&lt;&lt;</button>
						<button onclick="movePage('${path}/board/monlist?page=${pageInfo.prevPage}');" class="table-button" >&lt;</button>
						<c:forEach begin="${pageInfo.startPage}" end="${pageInfo.endPage}"
							...
							<c:if test="${status.current != pageInfo.currentPage}">
								<button class="table-button"
									onclick="movePage('${path}/board/monlist?page=${status.current}');">
									${status.current}</button>
							</c:if>
						</c:forEach>
						<button onclick="movePage('${path}/board/monlist?page=${pageInfo.nextPage}');" class="table-button" >&gt;</button>
						<button onclick="movePage('${path}/board/monlist?page=${pageInfo.maxPage}');" class="table-button" >&gt;&gt;</button>
					</div>
				</article>
			</div>
		</div>
